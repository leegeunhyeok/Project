...
def get_item_data(urls, thumbnail=False):
...
if __name__ == "__main__":
    ...
    process_count = 8
    links = []
    for i in range(1, max_page+1): # 1 ~ 최대 페이지까지
        page_start = time.time()
        print("----------[ %d 페이지 저작물 링크 추출 ]----------" % (i))
        links.append(get_links(base_list_url + str(i)))
            
        if len(links) == process_count or i == max_page:
            for i in links:
                p = multiprocessing.Process(target=get_item_data, args=(i, True))
                p.daemon = True
                process.append(p)
                p.start() # 프로세스 시작
