...
# 공유마당의 마지막 페이지 추출
def get_max_page():
    html = urllib.request.urlopen("https://gongu.copyright.or.kr/gongu/wrt/wrtCl/listWrt.do?menuNo=200023&wrtTy=4&depth2At=Y&pageIndex=1")
    source = html.read()
    soup = BeautifulSoup(source, "html.parser")
...
def get_item_data(ft, max_page, num, thumbnail=False):
...
if __name__ == "__main__":
    ...
    for c in get_count(max_page, process_count):
        # args:
        # c: 몇 페이지부터 몇개의 페이지를 추출할지
        # max_page: 크롤링 전체 페이지 수
        # num: 크롤링 한 저작물 갯수 카운트
        # True: 썸네일 이미지 생성 여부(False: 생성 안함)
        p = multiprocessing.Process(target=get_item_data, args=(c, max_page, num, True))
        process.append(p)
        p.start()
